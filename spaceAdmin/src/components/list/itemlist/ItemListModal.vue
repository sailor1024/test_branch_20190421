<template>
  <div class="itemList-dialog-container" tabindex="-1" @keyup.esc="closeModal" @click.self="closeModal" style="cursor:pointer;">
    <div class="itemList-dialog-box" style="cursor:default;">
      <div class="itemList-dialog-body">
        <div class="itemList-dialog-content">
          <!-- title -->
          <!-- <h3 class="itemList-dialog-body-title">添加合作者到"{{this.inviteProject.title}}"</h3> -->
          <!-- tips -->
              ...
              <div class="itemList-dialog-detail-item-content">
                ...
                <div v-else>
                  <ul class="itemList-dialog-shareTo">
                    ...
                    <li v-if="!data.length" class="itemList-dialog-shareItem"><span>目前没有邀请合作者</span></li>
                  </ul>
                </div>
              </div>
            </div>
...
export default {
  ...
  created () {
    ...
    this.$http.post('index2/index/invite_cooperator_list', {
      'item_dir': '',
      'project_id': this.inviteProject.id
    }).then((res) => {
      let data = res.data
      if (data.code === 200) {
        this.data = data.data
      ...
      if (this.email) {
        if (!/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(this.email)) {
          ...
          this.$http.post('index2/email/email', {
            ...
            'item_id': this.inviteProject.id,
            'edit_type': this.inviteProject.edit_type
          }).then((res) => {
