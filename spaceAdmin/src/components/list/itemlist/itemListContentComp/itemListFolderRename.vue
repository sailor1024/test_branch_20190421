  ...
  <div class="dialog_box">
    <div class="dialog_body">
      <div class="dialog_content">
        <!-- title -->
        <h3 class="dialog_body-title"><span style="font-size: 30px;">重命名</span></h3>
        <!-- detail -->
        <div class="dialog_body-detail">
          <input v-focus type="text" autofocus v-model="name" placeholder="请输入名字" style="border:0; font-size:16px;line-height: 1.15" class="create-input" @focus="focus" @keyup="show" @blur="blur">
...
export default {
  ...
  mounted () {
    this.name = this.FolderRenameMessage.dir_name
  },
  ...
  methods: {
    ...
    yes: function () {
      this.$Loading.start()
      this.$http.post('index2/file/redirname', {
        // 文件夹id
        company_id: this.$store.state.userInfo.company_id,
        user_id: this.$store.state.userInfo.id,
        dir_id: this.FolderRenameMessage.id,
        // 传递用户修改的名字
        dir_name: this.name
      }).then((res) => {
