...
class Browser {
    ...
    /* public function get_DevicesDetection_getOsFamilies( $entity_param = []){
        //设置默认天数
        $now_time = time();
    ...
    public function get_DevicesDetection_getOsVersions($entity_param = [] ) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d", $now_time  );
        $yesterday_time = date("Y-m-d", ($now_time - 60*60*24)); //昨天
    ...
    //获取浏览器名称
    public function get_DevicesDetection_getBrowsers($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d", $now_time);
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    //获取浏览器引擎
    public function get_DevicesDetection_getBrowserEngines($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d", $now_time  );
        $yesterday_time = date("Y-m-d", ($now_time - 60*60*24)); //昨天
    ...
    //获取浏览器插件
    public function get_DevicePlugins_getPlugin($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d", $now_time  );
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    //客户端配置 （分辨率与型号） 本周数据
    public function get_Resolution_getConfiguration($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d", $now_time );
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
