...
class Data {
   ...
   //如果对接口的操作的返回不明确，登录到后台  index.php?module=API&action=listAllAPI&date=yesterday&period=day&idSite=9
    ...
    /* public function Live_getVisitorProfile($entity_param = [])
    {
    ...
    //获取总访客信息,比较昨天和今天
    public function all_VisitsSummary_get($entity_param = []) {
        //设置默认天数
        $now_time = time();
    ...
    //获取总访客信息，历史访客
    public function history_VisitsSummary_getVisits($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d",$now_time);
       // $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    public function active_users_VisitorInterest_other($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d",$now_time);
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    public function active_users_VisitorInterest($entity_param = []) {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d",$now_time);
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    //获取访客信息, 当前周的每日访客量
    public function one_day_week_VisitsSummary_getVisits($entity_param = []) {
        $return_data = array(
            'data' => [],
            'message' => '',
            'code'=> 201,
    ...
    /*  
    public function areal_UserCountry_getCity($entity_param = []){
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d",$now_time);
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    */
     ...
     //获取某个项目访问量
     public function get_VisitsSummary_getVisits($entity_param = []) {
        //$entity_param = input('param.');
    ...
    */
    public function items_send_sum($entity_param = []) {
        $return_data = array(
            'data' => [],
            'message' => '',
            'code'=> 201,
        ...
        try {
            ...
            if (empty($entity_param['get_data_type'])) { //如果有该参数可能是user或者company的数据,或者items
                throw new \Exception('请添加 get_data_type');
            } else {
                if ($entity_param['get_data_type']  == 'user' ||  $entity_param['get_data_type']  == 'company') {
                    
                } else {
                    throw new \Exception('填写的 get_data_type "'.$entity_param['get_data_type']  .'"  有误');
                }
            }
           
            //$entity['_'] = '5d44b0f9529322dbdb72289bef206447';
            if (!empty($entity_param['_'])) {
                $userData = Token::getUserGreps(['key_name' => $entity_param['_']]);
                
                if ($entity_param['get_data_type'] == 'company') {
                    $param_get_sum['where']['company_id'] =  $userData['company_id'];
                } else if ($entity_param['get_data_type'] == 'user') {
